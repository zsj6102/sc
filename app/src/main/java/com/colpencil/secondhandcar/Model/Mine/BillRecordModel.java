 ...
 */
public class BillRecordModel implements IBillRecordModel {

    private Observable<Result<BillRecord>> observable;

    @Override
    public void billRecord(HashMap<String, String> params) {
        observable = RetrofitManager.getInstance(1, CarApplication.getInstance(), UrlConfig.BASE_HOST_)
                .createApi(CarApi.class)
                .billRecord(params)
                .subscribeOn(Schedulers.io())
                .map(new Func1<Result<BillRecord>, Result<BillRecord>>() {
                    @Override
                    public Result<BillRecord> call(Result<BillRecord> recordResult) {
                        return recordResult;
                    }
    ...
    @Override
    public void sub(Subscriber<Result<BillRecord>> subscriber) {
